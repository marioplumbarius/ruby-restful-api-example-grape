shared_examples_for :cannot_connect_error do |method, *args|

  context 'when it cannot connect to redis server' do

    before do
      allow(redis).to receive(method).and_raise(Redis::CannotConnectError)
    end
    ...
    context 'with silent set to false' do
      ...
      it 'raises Redis::CannotConnectError error' do
        expect { instance.method(method).call(*arguments) }.to raise_error(Redis::CannotConnectError)
      end
    end
    ...
    context 'with silent set to true' do
      ...
      it 'does not raise Redis::CannotConnectError error' do
        expect { instance.method(method).call(*arguments) }.not_to raise_error
      end
    end
